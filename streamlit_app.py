...
with tabs[1]:
    ...
    st.markdown(
        ...
        - 실제 학교나 가정의 데이터를 사용하려면 동일한 컬럼 구조(`date`, `value`, `group`)의 CSV를 확보해 위 코드를 수정하거나, 향후 업로드 기능을 추가하면 됩니다.
        - 권장 데이터 컬럼: `date`(ISO 8601), `value`(숫자, PM2.5 µg/m³), `group`(교실/실외 등)
        """
    )
...
# ---------------------------
st.markdown("---")
st.caption("참고: 본 대시보드는 교육·데모 목적의 예시 구현입니다. 실제 의학·보건 권고는 WHO 및 지역 보건당국 권고를 따르세요.")
